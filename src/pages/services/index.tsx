...
function ServiceListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ServiceInterface[]>(
    ...
    () =>
      getServices({
        relations: ['garage', 'appointment.count'],
      }),
  );
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('garage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>garage</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('garage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/garages/view/${record.garage?.id}`}>
                          {record.garage?.name}
                        </Link>
                      </Td>
                    )}
