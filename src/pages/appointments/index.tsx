...
function AppointmentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AppointmentInterface[]>(
    ...
    () =>
      getAppointments({
        relations: [
          'user_appointment_customer_idTouser',
          'service',
          'user_appointment_receptionist_idTouser',
          'task.count',
        ],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_customer_idTouser</Th>
                  )}
                  {hasAccess('service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>service</Th>}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_receptionist_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_customer_idTouser?.id}`}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_receptionist_idTouser?.id}`}>
                          {record.user_appointment_receptionist_idTouser?.email}
                        </Link>
                      </Td>
                    )}
