...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'mechanic', 'receptionist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GarageInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/garages` : null),
    () => getGarages({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Garage',
      path: isTenantUser ? `/garages/view/${data?.[0]?.id}` : '/garages',
      entity: 'garage',
